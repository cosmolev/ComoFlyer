...
public class SuperRenderer extends LwjglRenderer {

    private static final Logger logger = Logger.getLogger(SuperRenderer.class.getName());
...
//                logger.log(Level.FINER, "FBO Max MRT renderbuffers: {0}", maxMRTFBOAttachs);
    ...
    public void applyRenderState(RenderState state) {
        ...
        if(state.getAlphaFallOff() != this.context.alphaTestFallOff) {
            GL11.glAlphaFunc(516, this.context.alphaTestFallOff);
            this.context.alphaTestFallOff = state.getAlphaFallOff();
        }
        ...
        if(this.context.stencilTest != state.isStencilTest() || this.context.frontStencilStencilFailOperation != state.getFrontStencilStencilFailOperation() || this.context.frontStencilDepthFailOperation != state.getFrontStencilDepthFailOperation() || this.context.frontStencilDepthPassOperation != state.getFrontStencilDepthPassOperation() || this.context.backStencilStencilFailOperation != state.getBackStencilStencilFailOperation() || this.context.backStencilDepthFailOperation != state.getBackStencilDepthFailOperation() || this.context.backStencilDepthPassOperation != state.getBackStencilDepthPassOperation() || this.context.frontStencilFunction != state.getFrontStencilFunction() || this.context.backStencilFunction != state.getBackStencilFunction()) {
            ...
            if(state.isStencilTest()) {
                GL11.glEnable(2960);
                GL20.glStencilOpSeparate(1028, this.convertStencilOperation(state.getFrontStencilStencilFailOperation()), this.convertStencilOperation(state.getFrontStencilDepthFailOperation()), this.convertStencilOperation(state.getFrontStencilDepthPassOperation()));
                GL20.glStencilOpSeparate(1029, this.convertStencilOperation(state.getBackStencilStencilFailOperation()), this.convertStencilOperation(state.getBackStencilDepthFailOperation()), this.convertStencilOperation(state.getBackStencilDepthPassOperation()));
                GL20.glStencilFuncSeparate(1028, this.convertTestFunction(state.getFrontStencilFunction()), 0, 2147483647);
                GL20.glStencilFuncSeparate(1029, this.convertTestFunction(state.getBackStencilFunction()), 0, 2147483647);
            } else {
                GL11.glDisable(2960);
            }
        }
        ...
        if (fb == null) {
            ...
            if (fb.getNumColorBuffers() == 0) {
                ...
                if (fb.getNumColorBuffers() > maxFBOAttachs) {
                    throw new RendererException("Framebuffer has more "+fb.getNumColorBuffers()+" color "
                            + "attachments than are supported"
                            + " by the video hardware!"+maxFBOAttachs);
                }
                if (fb.isMultiTarget()) {
